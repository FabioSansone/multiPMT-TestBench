...
class Client:
    ...
    def __init__(self, port, server_ip, client_id, bme, tla, hv, hv_port = '/dev/ttyPS1'):
    ...
    def read(self, addr):
        if (self.checkRegBoundary(self.auto_int(addr))):
            addr = self.auto_int(addr)
            value = int.from_bytes(self.regs[addr*4:addr*4+4], byteorder='little')
    ...
    def check_feb(self):
        valid, not_valid = self.hv.processChannels(CHANNELS)
