...
class HV():
    ...
    def set_pmt_serials(self, channels, port, serials):
        """Function to set the serial number for each PMT"""
        list_channels = self.get_channels(channels)
        for channel in list_channels:
            logger_hv.info(f'Setting serial number for channel {channel}')
            if self.open(port, channel):
        ...
        for hv in hv_list:
            if self.open(port, hv):
                try:
                    ...
                    info[hv-1] = [device_id, pmt_serial, feb_serial, hv_serial]
